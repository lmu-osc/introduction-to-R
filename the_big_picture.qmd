...
If we only want to show the first few rows
...
On the left we can see the columns of this data.frame, named `Category`, `Year`, `Athlete` and `Nationality`. On the right we see the first values in each column, for example `1981`, `1981`, `1982` etc ... in the `Year`-column.
...
### Merging data
Sadly, the finishing times are not saved in this data set, but in another one named `times.rda`. 

Exercise: Load the second data set called "times.rda" into R on your own.

```{r, echo = FALSE}
load(file = "./raw_data/times.rda")
```

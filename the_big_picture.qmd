...
Now, lets do something with this data. We want to look at the most common name in every year and make a nice plot out of it.
Sadly the data is not complete. The n is missing (ok, i split it up for illustrational purposes).
When starting to work with data it is always a good idea to look at missing values, so let's do that quickly:
...
### Signifiance of one name vs. the other

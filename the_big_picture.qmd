...
[Packages](\packages.qmd) are extensions to the `base R` you get by default. We already installed our first packages in [About this workshop](https://nickhaf.github.io/r_tutorial/setup.html#about-this-workshop). Let's keep doing that and install the following package as well:
...
On the left we can see the columns of this data.frame, named `Date`, `Year`, `Applicants` etc ... On the right we see the first values in each column, for example `1981`, `1982` etc ... in the `Year`-column.

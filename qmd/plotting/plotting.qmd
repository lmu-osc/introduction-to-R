...
Looking pretty boring, right? Let's give each country another color by defining the `fill` aesthetic:
```{r}
p <- p +
...
This gives us black points now. If we had defined our aesthetics in the `ggplot()` function, the points would have the same colors as the bars:
```{r}
p +
  geom_point()
```

...
In the end, we want to plot the number of gold medals the countries have won on a world map. To do that, we need a data set containing coordinates of the different countries. Luckily, `ggplot2` (part of the `tidyverse`) provides a fitting data set. Let's [download](https://github.com/lmu-osc/introduction-to-R/tree/main/raw-data) it and load it into R:

```{r}
world_coordinates <- readRDS(file = here::here("raw-data", "world-coordinates.rds"))
```

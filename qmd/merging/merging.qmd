...
Let's take a quick look before to see, if there are any countries named differently in both data sets:

```{r}
medal_countries$Region[!(medal_countries$Region %in% world_coordinates$region)]
```

Looks like all of the countries in our `medal_countries` data frame can be also found in our `world_coordinates` frame. So let's merge them:

...
for (i in c("M", "W")) {
  ...
  ## Filter the gender i is at at the moment
...
### 2)
Do a paired t-test comparing the winning teams height vs the losing team height. Again use a [`for-loop`](../loops/loopq.qmd) to test for men and women separately. Save the result in a [list](https://nickhaf.github.io/r_tutorial/qmd/data_structures/data_structures.html#honorable-mention-list)), and name the list.
...
### 1)
Reform the `vb` data frame (with both men and women in it) into [long format](../format/format.qmd), so all heights can be found in one column, with the winning/losing information in another column.

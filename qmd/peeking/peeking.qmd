...
Here, the column names are printed on the left side, followed by the [type](https://lmu-osc.github.io/introduction-to-R/qmd/data-structures/data-structures.html#data-types) of the column and then the first few values of each column. We can also see at the top that this object is a data frame with `r nrow(athletes)` rows and `r ncol(athletes)` columns.

...
Before starting to do something with your data, it is always a good idea to get an overview. 
Our goal is to answer questions in the line of:

1. Which variables does our data have?
1. How many rows/columns does our data frame have? If we have a list, how long is it, what is saved within?
1. What types are our variables? Do we have to transform them, before we can work with them?
1. Do we have any [missing values](../missing/missings.qmd)?
...
str(winners)
```
Here, the column names are printed on the left side, followed by the [type]() of the column and then the first few values of each column.
We can also see in the first row that this object is a data frame with `165` rows and `7` columns.
...
For numeric columns we can see some distributional properties, like minimum and maximum, median and mean, as well as the first and third quantile. In case of [missing values](../missings/missings.qmd) (`NAs`) their number is printed at the bottom (look at the `Hours` column). 
...
We start with the function `ggplot()`, where we define our data and the x and y axes. This will draw our (empty) coordinate system. 
We have to use the [`aes()`](http://localhost:7280/qmd/plotting/plotting.html#aes) (aesthetics) function for everything that changes in relation to the data. The simplest example would be our axes, as the exact location of each element in the plot is dependent on the x and y position deposited in the data frame: 

...
To remove them, we have to bring together multiple concepts we have already looked at in previous chapters:

```{r, output.lines = 4}
winners_na <- winners[!is.na(winners$Hours), ]
```

What happens here? Like always when [filtering specific rows](), we define a [logical vector](https://nickhaf.github.io/r_tutorial/qmd/basics/basics.html#comparisons-and-logical-operators), which has a `TRUE` for all rows that have a missing on `ID` and a `FALSE` for all others (by using the `!` operator, which inverts the boolean values - otherwise we would extract all rows with missing values in the `Hours` column):

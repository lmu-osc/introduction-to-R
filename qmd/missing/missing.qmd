...
## What are `NA's`?
Remember the weird `NA` rows we have encountered when [subsetting by condition](https://lmu-osc.github.io/introduction-to-R/qmd/subsetting/subsetting.html#conditional-filtering)? We were able to steer around that by using `filter()`, but let's take a closer look at that now.
...
athletes[
  (athletes$Sport == "Judo") &
    (athletes$Weight > 100 | athletes$Weight < 50) &
    !is.na(athletes$Weight),
]
```

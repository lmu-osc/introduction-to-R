...
Print each fictional universe (column: `uni_name`) in the `characters_stats` data frame into your console once, like this:
`"The fictional universe 'fictional universe' is part of the characters data set."`
...
## Exercise 2
Remember how we used the `group_by()` command to [calculate the number of gold medals for each country](https://nickhaf.github.io/r_tutorial/qmd/merging/merging.html#before-merging)?
Well, now you know enough to do something similar without using the tidyverse, but just a `for-loop`. 
...
## Hint
Build a `for loop` that goes over all unique questions in your subsetted data frames. Inside this `for-loop` you can subset again, this time only the rows containing the question that the loop is at at the moment, and calculate its mean rating from here. 
Then use `paste()` to build and print the statement. 
:::
...
for(i in unique(characters_friends$question)){ # goes over all unique questions
  ...
  question_mean <- mean(question_dat$mean_rating)
...
1. Now you can't easily loop over the question column itself, because you need the position of each element to save it in a new data frame: `for(i in 1:length(unique(characters_friends$question))){`.
1. Because of that you can save the result of your calculateion in row `i` of your new data frame. 
...
characters_friends %>%
  ...
  filter(question == 'beautiful/ugly')
```
:::

Great, its the same! So it ist propably a lot easier to just use `group_by()` with `summarise()`, but maybe not quite as instructive. 

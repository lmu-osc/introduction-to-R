...
Mainly we will use these logical operation to check which elements in a [vector](https://nickhaf.github.io/r_tutorial/data_structures.html#vector) satisfy some requirements:
...
```
will rep `4` `10` times. 
If we swap the arguments, the `10` will be repeat `4` times:
```{r}
rep(10, 4)
```
...
In the next chapter we will take a quick look at R's package infrastructure which makes it easy to add a multitude of additional functionality.

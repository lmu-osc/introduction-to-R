...
str(log_vec)
```
The first one is `num` (numeric) so it only stores numeric values. The second one is `char` (character), so it only can contain strings. And last but not least we have `logi` (logical) for [boolean values](https://nickhaf.github.io/r_tutorial/basics.html#comparisons-and-logical-operators).
Why is that important? 
Well, some functions only make sense for specific data types. For example:
...
We won't use it any more in this workshop, but keep in mind it exists, as it quickly becomes necessary for managing more complex tasks.  
